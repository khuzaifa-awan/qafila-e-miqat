...
export function PackageCard({ 
  ...
  const isBookingDisabled = (pkg.seatsLeft ?? 0) === 0;
    ...
    <Card className="group relative overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
        ...
        <div className="space-y-4">
          ...
          <div className="border-t pt-4">
            <div className="flex items-center justify-between">
              ...
              <div className={`text-center text-sm font-semibold px-6 py-1 rounded-full ${
                isBookingDisabled 
                  ? 'bg-gray-200 text-gray-500' 
                  : 'bg-[#F7E9D9] text-[#AD5628]'
              }`}>
                {isBookingDisabled ? (
                  <>
                    Sold Out
      ...
      <CardFooter className="p-6 pt-0 space-y-3">
        ...
        {isBookingDisabled ? (
          <Button 
            disabled
            className="w-full relative overflow-hidden rounded-lg bg-gray-400 !text-gray-200 py-2 font-semibold cursor-not-allowed" 
            size="lg"
          > 
            Sold Out - Booking Unavailable
          </Button>
        ) : (
          <Link 
            href={`/booking-form?packageId=${pkg.id}`}
